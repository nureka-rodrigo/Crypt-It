...
export const AESCTR = () => {
  ...
  const AESDecode = async (cipherText: string, keyStr: string, counterStr: string) => {
    const key = await importKey(keyStr);
  ...
  const onEncode = async (data: EncodeFormData) => {
    const {cipherText, counter} = await AESEncode(data.plainText, data.encodeKey);
      ...
      <section className="max-w-7xl py-8 space-y-8 mx-auto">
        <div className="container mx-auto">
          ...
          <p className="mt-4 text-neutral-700 dark:text-neutral-300 text-justify">
            The Advanced Encryption Standard in Counter mode (AES-CTR) is a powerful and flexible encryption method used
            to secure data. Unlike traditional block ciphers that operate on fixed-size blocks of data, AES-CTR
            transforms AES into a stream cipher, allowing for more efficient and flexible encryption of data of
            arbitrary length.
          </p>
          <p className="mt-4 text-neutral-700 dark:text-neutral-300 text-justify">
            In AES-CTR, a unique counter value is combined with a secret key to produce a stream of pseudo-random data,
            which is then XORed with the plaintext to produce the ciphertext. The same process, using the same counter
            and key, can be reversed to decrypt the ciphertext back into the original plaintext. The counter ensures
            that even identical blocks of plaintext produce different ciphertexts, enhancing security.
          </p>
          <p className="mt-4 text-neutral-700 dark:text-neutral-300 text-justify">
            AES-CTR is widely used in various applications due to its speed and simplicity. It does not require padding,
            and its parallelize nature makes it suitable for high-performance scenarios. However, it's crucial to
            manage the counter correctly, ensuring it is unique for each encryption operation to maintain security.
          </p>
        </div>
